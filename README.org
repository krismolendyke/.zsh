...
*** TODO =PROMPT_COMMAND=
    ...
    #+BEGIN_SRC sh
      function k20e_kubectx() {
          ...
          declare -r SUFFIX="${RESET} "
          local value=''
          if [[ $KUBECONFIG == *kind-config-kind ]]; then
              value="${PREFIX}local${SUFFIX}"
...
** pyenv
   ...
   I use this to avoid having to track latest Python from Homebrew
   since almost every other recipe depends on it.  Having it get
   updated all the time screws up all the virtual environments,
